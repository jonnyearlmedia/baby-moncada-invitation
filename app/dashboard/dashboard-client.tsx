...
export default function DashboardClient() {
  ...
  return <main className="admin-shell"><header className="admin-top"><div><p className="admin-kicker">Baby Moncada</p><h1>Host dashboard</h1><p>Manage invitation links, guest names, event details, and live RSVP results.</p><p className="dashboard-save-help"><strong>Save the host dashboard</strong><span>On Safari or Chrome, tap Share → Add to Home Screen for quick access to invitation links and live RSVPs.</span><small>Prefer a bookmark? Use Add Bookmark in Safari or Add to Bookmarks in Chrome.</small></p></div><button className="admin-secondary" onClick={async () => { await fetch("/api/admin/session", { method: "DELETE" }); router.replace("/dashboard/login"); }}>Log out</button></header>
