...
export default function DashboardClient() {
  ...
  return <main className="admin-shell"><header className="admin-top"><div><p className="admin-kicker">Baby Moncada</p><h1>Host dashboard</h1><p>Manage invitation links, guest names, event details, and live RSVP results.</p><p className="dashboard-save-help"><strong>Save this dashboard:</strong> Safari or Chrome — Share → Add to Home Screen. For a browser bookmark, use Safari’s Add Bookmark or Chrome’s Add to Bookmarks. The saved icon opens this dashboard directly.</p></div><button className="admin-secondary" onClick={async () => { await fetch("/api/admin/session", { method: "DELETE" }); router.replace("/dashboard/login"); }}>Log out</button></header>
