...
A read is only published when it agrees with Amazon's own `<purchased>/<total> items purchased` header. That count is the one independent number on the page, and it is the only check that catches a page Amazon silently skipped, because every page that was read still looks valid on its own. A filter that comes back completely empty is treated as a failed read unless that header says the filter really is empty. An empty page part way through a filter is treated as the end of that filter even when Amazon still hands back a pagination key, which it does: reading that as a broken page left the registry stale for two days in September.
...
The scheduled job is `.github/workflows/amazon-registry-sync.yml`. Its three GitHub Actions secrets are `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`, and `REGISTRY_SYNC_TOKEN`. A failed run is logged in `registry_sync_runs`, leaves the prior snapshot untouched, uploads the page Amazon returned as a diagnostics artifact, and opens a `registry-sync` tracking issue that later failures comment on and the next success closes. Run the workflow manually with `dry_run` enabled to check Amazon without touching the live snapshot.
